  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data: project,
        error: projectError
      } = await supabase.from('projects').select('client_admin_id').eq('id', projectId).single();
      ...
      const {
        ...
        `).eq('id', project.client_admin_id).eq('user_role_id', 3) // client_admin role
      .single();
      if (adminError) {
      ...
      } = await supabase.from('project_assignees').select(`
          user:user_profiles(
            id,
            first_name,
            last_name,
      ...
      const allTeamMembers = [clientAdmin, ...(assignees?.map(a => a.user) || [])].filter(Boolean);

      // Remove duplicates
      return allTeamMembers.filter((member, index, self) => index === self.findIndex(m => m.id === member.id));
    }
  });
  if (isLoading) {
  ...
  return <Card className="p-6">
      ...
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {teamMembers && teamMembers.length > 0 ? teamMembers.map(member => <TableRow key={member.id}>
                  <TableCell>

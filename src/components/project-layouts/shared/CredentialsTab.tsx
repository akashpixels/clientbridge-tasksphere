...
const CredentialCard = ({
  ...
  credential: Tables<"project_credentials">;
}) => {
  const [showPassword, setShowPassword] = useState(false);
  const copyToClipboard = (text: string) => {
    navigator.clipboard.writeText(text);
  };
  return <Card className="relative flex flex-col h-[320px] hover:shadow-md transition-shadow duration-200 overflow-hidden group">
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0">
              ...
              <a href={credential.url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-500 hover:text-blue-600 truncate block">
                {credential.url}
              </a>
            </div>
          </div>
          
          {credential.username && <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-grow">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => copyToClipboard(credential.username)}>
          ...
          {credential.password && <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-grow">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="ghost" size="sm" className="h-7 w-7 p-0 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => copyToClipboard(credential.password)}>
                    <Copy className="h-4 w-4 text-muted-foreground" />
                  </Button>
                </div>
              </div>
...
const CredentialsTab = ({
    ...
    queryFn: async () => {
      try {
        ...
        const {
          data,
          error
        } = await supabase.from('user_profiles').select('user_role_id').eq('id', user.id).single();
        if (error) {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from('project_credentials').select('*').eq('project_id', projectId).order('created_at', {
        ascending: false
      });
      if (error) {
  ...
  return <Card className="p-6">
      {isLoading ? <div>Loading credentials...</div> : credentials && credentials.length > 0 ? <div className="space-y-6">
